 ...
 */
class ImageGridLayout @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        targetRowHeight = a.getDimensionPixelSize(
    ...
    private fun View.aspectRatio(): Float =
        (layoutParams as? LayoutParams)?.aspectRatio?.takeIf { it > 0f } ?: 1f
     ...
     */
    private fun layoutJustifiedRows(children: List<View>, available: Int): Int {
        val count = children.size
        var y = paddingTop
        var rowStart = 0
        while (rowStart < count) {
            ...
            val rowHeight = if (isLastRow && rowFilledWidth < available) {
                targetRowHeight
            } else {
