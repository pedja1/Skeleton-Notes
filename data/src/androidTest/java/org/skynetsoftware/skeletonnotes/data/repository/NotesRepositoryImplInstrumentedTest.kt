...
class NotesRepositoryImplInstrumentedTest {
    ...
    @Test
    fun getAllNotesReturnsAllSavedNotesFlow() =
        runTest(timeout = 5.seconds) {
            repository.saveNote(
                NoteWithAttachments(
                    note =
    ...
    @Test
    fun getAllNotesReturnsEmptyListWhenNoNotesFlow() =
        runTest(timeout = 5.seconds) {
            val result = repository.getAllNotesFlow().first()
    ...
    @Test
    fun deleteNonExistentNoteDoesNotThrow() =
        runTest(timeout = 5.seconds) {
    ...
    @Test
    fun getNoteByIdForNonExistentNoteThrowsInDataSource() =
        runTest(timeout = 5.seconds) {
    ...
    @Test
    fun saveNoteWithAttachmentsStoresAttachments() =
        runTest(timeout = 5.seconds) {
            val noteId = UUID.randomUUID().toString()
            val note =
                Note(
    ...
    @Test
    fun notesAreSortedByModifiedAtDescending() =
        runTest(timeout = 5.seconds) {
            repository.saveNote(
                NoteWithAttachments(
                    note =
